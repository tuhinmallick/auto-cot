...
class Decoder():
    ...
    def decode(self, args, input, max_length):
      return decoder_for_gpt3(args, input, max_length)
  ...
  if args.dataset == "aqua":
    with open(args.dataset_path) as f:
      ...
      for line in lines:
        ...
        choice = f"Answer Choices:{choice}"
...
def create_demo_text(args, cot_flag):
  ...
  return "".join(
      ...
      ".\n\n" if cot_flag else
      f"{x[i]} {args.direct_answer_trigger_for_fewshot} {y[i]}" + ".\n\n"
      for i in index_list)
...
def answer_cleansing_zero_shot(args, pred, must_choice=False):
  ...
  if args.dataset in ("aqua", "commonsensqa"):
    ...
    # If there is no candidate in list, null is set.
  pred = "" if len(pred) == 0 else pred[0]
  # (For arithmetic tasks) if a word ends with period, it will be omitted ...
  if pred != "":
      if pred[-1] == ".":
          pred = pred[:-1]
